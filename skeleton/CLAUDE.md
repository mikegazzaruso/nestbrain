...
You are working inside a **NestBrain**: a personal knowledge + project workspace managed by [MindNest](https://github.com/mikegazzaruso/MindNest). Think of it as a folder-based OS where notes, compiled knowledge, and real code projects live side by side in one integrated environment.
...
To find the NestBrain root: starting from your current working directory, walk **up** the directory tree until you find a directory that contains **all of**: `CLAUDE.md` (this file), `.mindnest/`, `Daily/`, and `Skills/`. That directory is `<nestbrain_root>`.

**Every path mentioned in this document and in the session skills** — `Daily/`, `Skills/`, `Projects/`, `Library/Knowledge/`, `.mindnest/`, etc. — resolves against `<nestbrain_root>`, never against your current working directory. When you read, write, or list any of these, use the absolute path `<nestbrain_root>/<path>`.

## Prime directive

- `Library/Knowledge/` and `.mindnest/` belong to **MindNest**. Everything else belongs to the **user**.
- Never edit files inside `Library/Knowledge/` by hand — it is a compiled wiki, regenerated by MindNest from raw sources.
- Never touch `.mindnest/` — it holds internal state (raw sources, vector index, settings).
- If the user wants to add knowledge to the wiki, use `mindnest ingest <url|file>` followed by `mindnest compile`.

## Directory map

| Path                     | Owner              | Purpose                                                                 |
|--------------------------|--------------------|-------------------------------------------------------------------------|
| `Library/Knowledge/`     | MindNest           | Compiled wiki (Obsidian-compatible vault). Regenerable. Read-only.      |
| `.mindnest/`             | MindNest           | Raw sources, vector index, settings. Never modify.                      |
...
| `CLAUDE.md` (this file)  | MindNest           | Workspace orientation. You are reading it.                              |
...
- **For knowledge queries**: prefer `mindnest ask "..."` over reading wiki files directly — it uses hybrid semantic search and returns cited answers.
- **For adding knowledge**: `mindnest ingest <source>`, then `mindnest compile`. Never write into `Library/Knowledge/` by hand.

## MindNest CLI

```
mindnest ingest <source>       # URL, file, GitHub repo, arXiv, YouTube, RSS
mindnest compile               # Compile wiki from new/changed sources (incremental)
mindnest ask "question"        # Query the wiki with hybrid search + citations
mindnest search "query"        # Hybrid semantic + keyword search
mindnest lint                  # Wiki health check
...
- Never delete `.mindnest/raw/` — user-provided source material, not regenerable.
- Never edit files in `Library/Knowledge/` directly — the wiki is regenerable via `mindnest compile`, but hand-edits will be overwritten.
- The wiki *is* regenerable. Raw sources and user notes are *not*. When in doubt about ownership, ask.

---

*MindNest — by [Mike Gazzaruso / NextEpochs](https://github.com/mikegazzaruso). Learn more: https://github.com/mikegazzaruso/MindNest*
